 ...
 *  https://github.com/drpioneer/MeteoStationESP8266
 *  Станция собирает информацию от метеодатчиков и отсылает информацию на сервер народного мониторинга (narodmon.ru)
 *  При первом запуске или отсутствии сети создается точка доступа на несколько секунд для возможности подключения 
 *  и настройки. В WEB-интерфейсе можно просканировать сети и подключится к имеющейся либо
 *  вписать название сети вручную, после чего на сервер народного мониторинга начинают отправлятся метеоданные, 
 *  с информацией о напряжении питания микроконтроллера и уровне сигнала Wifi. В виртуальный последовательный
 *  порт выводится отладочная информация о работе метестанции.
 *-------------------------------------------------------------------------------------------- */
#include <FS.h>
